...
class IndicatorValidator:
    ...
    def _save_text_report(self, df: pd.DataFrame, output_dir: str) -> None:
        """Save a simple text report of indicator values when visualization fails."""
        try:
            report_file = os.path.join(output_dir, "indicator_values_report.txt")
            sample_df = df.iloc[-20:].copy()  # Get last 20 rows

            with open(report_file, 'w') as f:
                f.write("INDICATOR VALUES REPORT\n")
                f.write("======================\n\n")
                f.write(f"Report generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")

                # Write basic price data
                f.write("PRICE DATA:\n")
                f.write("-----------\n")
                for idx, row in sample_df.iterrows():
                    date = row['time'].strftime('%Y-%m-%d') if isinstance(row['time'], pd.Timestamp) else str(
                        row['time'])
                    f.write(f"Date: {date}, Open: {row['open']:.2f}, High: {row['high']:.2f}, ")
                    f.write(f"Low: {row['low']:.2f}, Close: {row['close']:.2f}\n")

                # Write indicator groups
                indicator_groups = {
                    'Moving Averages': [col for col in sample_df.columns if
                                        col.startswith('sma_') or col.startswith('ema_')],
                    'MACD': [col for col in sample_df.columns if col.startswith('macd_')],
                    'Bollinger Bands': [col for col in sample_df.columns if col.startswith('bb_')],
                    'RSI': ['rsi'],
                    'Stochastic': ['stoch_k', 'stoch_d'],
                    'ATR': ['atr'],
                    'Pivot Points': ['pivot', 'support1', 'support2', 'resistance1', 'resistance2']
                }
                ...
                for group_name, cols in indicator_groups.items():
                    ...
                    if available_cols:
                        f.write(f"\n{group_name.upper()}:\n")
                        f.write("-" * len(group_name.upper()) + "\n")

                        # Write the most recent values (last row)
                        last_row = sample_df.iloc[-1]
                        for col in available_cols:
                            f.write(f"{col}: {last_row[col]:.4f}\n")

                self.logger.info(f"Text report saved to {report_file}")
        except Exception as e:
            self.logger.error(f"Error creating text report: {e}")

    def validate_technical_indicators(self, df: pd.DataFrame, report_dir: str = "ValidationReports") -> Dict[str, bool]:
        """Validate all technical indicators in the dataframe."""
        ...
        self._save_validation_report(report_dir)
    ...
    def generate_indicator_visualizations(self, df: pd.DataFrame,
                                          output_dir: str = "ValidationReports/Visualizations") -> None:
        """Generate visualization plots to help visually validate indicators."""
        os.makedirs(output_dir, exist_ok=True)
        self.logger.info(f"Generating indicator visualizations in {output_dir}")

        # Only use the last 100 data points for visualization to keep charts readable
        sample_df = df.iloc[-100:].copy()

        try:
            # 1. Price + Moving Averages
            self._plot_moving_averages(sample_df, output_dir)

            # 2. MACD
            self._plot_macd(sample_df, output_dir)

            # 3. Bollinger Bands
            self._plot_bollinger_bands(sample_df, output_dir)

            # 4. RSI
            self._plot_rsi(sample_df, output_dir)

            # 5. Stochastic Oscillator
            self._plot_stochastic(sample_df, output_dir)

            # 6. ATR
            self._plot_atr(sample_df, output_dir)

            # 7. Pivot Points
            self._plot_pivot_points(sample_df, output_dir)

            self.logger.info("Visualization generation complete")
        except Exception as e:
            self.logger.error(f"Error generating visualizations: {e}")
            # Save a simple text report instead
            self._save_text_report(df, output_dir)
    ...
    def _save_validation_report(self, output_dir: str) -> None:
        """Save validation results to a JSON file."""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_file = os.path.join(output_dir, f"validation_report_{timestamp}.json")

        # Convert validation results to a serializable format
